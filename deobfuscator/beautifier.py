...
def beautify_lua(code: str) -> str:
    ...
    # Step 3: Constant Folding (e.g., "a" .. "b" -> "ab")
    code = _constant_folding(code)
    
    # Step 4: Remove redundant logic (e.g., if true then)
    code = _remove_redundant_logic(code)
    ...
    # Step 7: Trim trailing whitespace on each line
...
def _constant_folding(code: str) -> str:
    """Merge concatenated strings and constants."""
    # Merge: "a" .. "b" -> "ab"
    pattern = r'["\']([^"\']*)["\']\s*\.\.\s*["\']([^"\']*)["\']'
...
def _remove_redundant_logic(code: str) -> str:
    """Remove useless obfuscation artifacts."""
    # Remove: if true then ... end
    pattern = r'if\s+true\s+then\s*\n?(.*?)\n?\s*end'
    code = re.sub(pattern, r'\1', code, flags=re.DOTALL)
    
    # Remove: do ... end
    # Note: be careful not to remove if it's for scope, but in obfuscation it's often junk
    # pattern = r'do\s*\n?(.*?)\n?\s*end'
    # code = re.sub(pattern, r'\1', code, flags=re.DOTALL)
